...
class LessonsContainer extends Component {
  ...
  render() {
    ...
    return (
      <LessonsList lessons={this.props.gradesToFilter.length > 0 || this.props.subjectsToFilter.length > 0 ?  this.filterLessonsBySubject().filter(lesson => lesson.title.toLowerCase().includes(this.props.searchBarInput.toLowerCase())) : this.props.lessons.filter(lesson => lesson.title.toLowerCase().includes(this.props.searchBarInput.toLowerCase()))} favoriteAction={this.props.favoriteAction} favoriteButtonText={this.props.favoriteButtonText} buttonColor={this.props.buttonColor} />
    )
  }
}
